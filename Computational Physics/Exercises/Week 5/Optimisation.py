...
class Potential():
    ...
    def _velocity_Verlet_1d(self, x, v, dt = 0.1, static_points = []):

        
        
        x_new = x + dt * v + dt**2 * 1/2 * self.force(x)
        
        v_new = v + 1/2 * (self.force(x) + self.force(x_new)) * dt
        return x_new, v_new

    def N_velocity_Verlet_1d(self, x0, v0, N, dt = 0.01, static_points = []):
        x = x0
        v = v0
        xs = []
        vs = []
        for _ in range(N):
            x, v = self._velocity_Verlet_1d(x = x, v = v, dt = dt, static_points = static_points)
            xs.append(x)
            vs.append(v)
        return xs, vs


    def constant_temp_MD_1d(self, x0, N = 10000, m = 1, dt = 0.01, static_points = []):
        x = x0
        
        xs = []
        for _ in range(N):
            v = np.random.normal(0,np.sqrt(self.kT/m))
            xs_new, vs_new = self.N_velocity_Verlet_1d(x0 = x, v0 = v, N = 50, dt = dt, static_points = static_points)
            x = xs_new[-1]
            xs += [x]

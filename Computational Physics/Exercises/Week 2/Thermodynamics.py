...
class Potential():
    ...
    def plot_all(self,ax, N_mcmc = 1000, color1 = 'C0', color2 = 'C1', color3 = 'C2'):
        self.plot_V(ax, color = color1)
        self.plot_P(ax, color = color2)
        self.plot_mcmc(ax, N= N_mcmc, color = color3)
    ...
    def plot_V(self, ax, color = 'C0'):
        ...
        ax.plot(x, self.V(x), label = f'{self.type} V(x)', color = color)
